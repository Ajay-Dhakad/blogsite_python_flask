...
def home():
    ...
    per_page = params['posts_per_page']  

    # Query the Posts table, sorting them by date in descending order
    posts = Posts.query.order_by(Posts.date.desc()).paginate(page=page, per_page=per_page)
...
def uploader():
    if ('user' in session and session['user']==params['admin_user']):
        if request.method=='POST':
            ...
            f.save(os.path.join(params['upload_folder'],(f.filename)))
